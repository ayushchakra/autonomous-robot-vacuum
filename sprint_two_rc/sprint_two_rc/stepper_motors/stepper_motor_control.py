...
MOTOR_FOUR_PINS = [38, 40]  # this is the step + direction pins

# Front right is using a different driver now, so ignore
...
def setup_all_pins():
    ...
    # setup front right pins separately
    GPIO.setup(MOTOR_FOUR_PINS[0], GPIO.OUT)
    GPIO.setup(MOTOR_FOUR_PINS[1], GPIO.OUT)
...
def run_motors(motor_one_pins=MOTOR_ONE_PINS, motor_two_pins=MOTOR_TWO_PINS, motor_three_pins=MOTOR_THREE_PINS, motor_four_pins=MOTOR_FOUR_PINS, time_sleep=TIME_SLEEP, revolutions=NUM_STEPS_PER_CMD):
    ...
    for i in range(revolutions):
        ...
        GPIO.output(MOTOR_FOUR_PINS[0], GPIO.HIGH)
        ...
        GPIO.output(MOTOR_FOUR_PINS[0], GPIO.LOW)
        time.sleep(time_sleep)
